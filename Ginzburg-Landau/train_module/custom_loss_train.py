...
def my_custom_loss_train(data_name, model_name, num_classes, train_loader, test_loader,
    ...
    # 加载模型
    model = get_model(model_name=model_name, in_features=num_classes).to(device)
...
def custom_loss(outputs, images, targets, model, alpha=1.0, beta=1.0):
    ...
    # 生成PGD噪声 默认20步长攻击
    iters = generate_pgd_noise(model, images, targets, adv_criterion, device,
    ...
    # 计算对抗损失
    robustness_loss = adv_criterion(adv_outputs, targets)
